...
- GitHub Actions provides verification for pushes and pull requests on every branch. Twilio deployment requires a manual `workflow_dispatch` run with `confirm_deploy: true` from `refs/heads/main`.
