...
blank private asset path, plus a malformed private asset export. It also covers
blank private asset message output before it can reach TwiML. The harness
XML-escapes local TwiML message bodies so special characters are represented
safely in the output.
...
- Private `/message.js` assets must export a function that returns a non-empty
  string from a non-blank asset path before `private-message` adds it to TwiML.
